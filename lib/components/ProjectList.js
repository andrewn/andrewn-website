...
const Title = ({ text }) => <p className={styles.title}>{text}</p>;

const Summary = ({ text }) => <p className={styles.summary}>{text}</p>;

const Award = ({ text }) =>
  text == null ? null : (
    <p className={`themeGradientOpacity ${styles.award}`}>{text}</p>
  );

const Image = ({ source }) =>
  source == null ? null : (
    <div className={styles.image}>
      <Figure source={source} aspectRatio="16/9" />
...
const Project = ({ work }: { work: Work }) => {
  ...
  if (link != null) {
    return (
      <LinkView href={link} className={classes}>
        {inner}
      </LinkView>
    );
  }
